...
void Usage(const char *progname)
{
  fprintf(stderr,"Usage: %s [options] original distorted\n"
	  ...
	  "--outside x y x2 y2: constrain up- or downsampling to an area outside of the given rectangle\n"
...
class Meter *ParseSubsampling(int &argc,char **&argv)
{
  ...
  if (!strcmp(arg,"--sub")) {
    ...
    down  = new class Downsampler(sx,sy,false);
    m     = down;
    argc -= 2;
    argv += 2;
